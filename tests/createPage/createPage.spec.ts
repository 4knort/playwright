...
test.describe('новый пользователь открывает страницу create', () => {
  ...
  test('видит панель с 10 элементами готовых темных цветов', async ({ page }) => {
    const items = await page.locator('.panel__colors >> nth=0 >> .panel__color-item')

    const items_count = await items.count();
    await expect(items_count).toEqual(10)
  })

  test('видит кнопку select all в панеле с темными цветами', async ({ page }) => {
  ...
  test('видит панель с 10 готовыми смешанными цветами', async ({ page }) => {
    const items = await page.locator('.panel__colors >> nth=1 >> .panel__color-item')

    const items_count = await items.count();
    await expect(items_count).toEqual(10)
  })

  test('видит кнопку select all в панеле со смешанными цветами', async ({ page }) => {
    const btn = await page.locator('.panel__btn >> nth=1')

    await expect(btn).toBeVisible()
  })

  test('видит кнопку открытие колор пикера в панеле смешанных цветов', async ({ page }) => {
    const colorPickerBtn = await page.locator('.panel__color-block')

    await expect(colorPickerBtn).toBeVisible()
  })

  test('видит что в панеле смешанных цветов на элементах нет крестика', async ({ page }) => {
    const items = await page.locator('.panel__colors >> nth=1 >> .panel__color-item >> .panel__color-icon')
    const items_count = await items.count();
  ...
  test('видит что в панеле темных цветов на элементах нет крестика', async ({ page }) => {
    const items = await page.locator('.panel__colors >> nth=0 >> .panel__color-item >> .panel__color-icon')
  ...
  test.describe(`в панеле темных цветов нажимает на кнопку select all`, () => {
    ...
    test(`в панеле выбора цветов все элементы окрасились соответствуя цветам в панеле темных цветов`, async ({ page }) => {
      const itemsChoose = await page.locator('.panel__color-item--choose');
      const itemsColor = await page.locator('.panel__colors >> nth=0 >> .panel__color-item')
    ...
    test.describe('нажимает на первый элемент в панеле смешанных цветов', () => {
      test.beforeEach(async ({ page }) => {
        const itemColor = await page.locator('div:nth-child(4) > .panel__colors > div:nth-child(2)');
        await itemColor.click()
        await wait(1000) // color changes to slow, playwright is too fast
      })
      test(' видит что последний элемент в панеле выбора цвета окрасился в тот же цвет', async ({ page }) => {
        const itemChoose = await page.locator('.panel__color-item--choose >> nth=9');
        const itemColor = await page.locator('div:nth-child(4) > .panel__colors > div:nth-child(2)');
  ...
  test.describe(`в панеле смешанных цветов нажимает на кнопку select all`, () => {
    ...
    test(`в панеле выбора цветов все элементы окрасились соответствуя цветам в панеле смешанных цветов`, async ({ page }) => {
      const itemsChoose = await page.locator('.panel__color-item--choose');
      const itemsColor = await page.locator('.panel__colors >> nth=0 >> .panel__color-item')
  ...
  test.describe(`наводит на элемент в панеле темных цветов`, () => {
    test.beforeEach(async ({ page }) => {
      await page.locator('div:nth-child(3) > .panel__colors > div').first().hover();
    });

    test(`видит что появился крестик`, async ({ page }) => {
      const span = await page.locator('div:nth-child(3) > .panel__colors > div > .panel__color-icon >> nth=0')
      await expect(span).toHaveCSS('display', 'block')
    })

    test.describe('убирает курсор с элемента', () => {
      test.beforeEach(async ({ page }) => {
        await page.locator('div:nth-child(3) > .panel__colors > div').last().hover();
      });
  ...
  test.describe(`наводит на элемент в панеле смешанных цветов`, () => {
    test.beforeEach(async ({ page }) => {
      await page.locator('div:nth-child(4) > .panel__colors > div:nth-child(2)').hover();
    });
    test(`видит что появился крестик`, async ({ page }) => {
      const span = await page.locator('div:nth-child(4) > .panel__colors > div:nth-child(2) > .panel__color-icon >> nth=0')
      await expect(span).toHaveCSS('display', 'block')
    })
  ...
  test.describe(`нажимает на первый элемент в панеле темных цветов`, () => {
    test.beforeEach(async ({ page }) => {
      await page.locator('div:nth-child(3) > .panel__colors > div').first().click();
      await wait(1000) // color changes to slow, playwright is too fast
    });

    test(`видит что первый элемент в панеле выбора цветов окрасился в тот же цвет`, async ({ page }) => {
      const itemChoose = await page.locator('.panel__color-item--choose >> nth=0');
      const itemColor = await page.locator('div:nth-child(3) > .panel__colors > div >> nth=0')
    ...
    test.describe('наводит на покрашенный элемент в выборе цветов', () => {
      ...
      test('видит что на нем появился крестик', async ({ page }) => {
        const span = await page.locator('text=Select up to 10 colors Select Colors by clicking on them+ >> span');
    ...
    test.describe('нажимает на покрашенный элемент в выборе цветов', () => {
      ...
      test('видит что он стал фон элемента стал прозрачным rgb(245. 245. 245)', async ({ page }) => {
        const colorItem = await page.locator('.panel__color-item--choose >> nth=0');
        await expect(colorItem).toHaveCSS('background-color', 'rgb(245, 245, 245)')
      })
    })
  })
